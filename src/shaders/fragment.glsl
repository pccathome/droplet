...
// Camera - 與 Three.js 相機保持一致
vec3 origin = vec3(0.0, 0.0, 10.0);
vec3 lookAt = vec3(0.0, 0.0, 0.0);
vec3 cDir = normalize(lookAt - origin);
vec3 cUp = vec3(0.0, 1.6, 0.0); // z 軸向上
...
float map(vec3 p) {
    ...
    float sphere = sdSphere(translate(p, vec3(0.0, 0.0, 0.0)), 0.55);
    d = smoothMin(d, sphere, k);
...
vec3 dropletColor(vec3 normal, vec3 rayDir) {
    ...
    // float intensity = 2.3;
    float intensity = 4.5 * (1.0 - dot(reflectDir, normal)) * (1.0 - dot(rayDir, normal));
...
void main() {
    ...
    vec3 finalColor = pow(color, vec3(1.0));
